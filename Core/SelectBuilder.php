...
class SelectBuilder
{
  ...
  private function addCondition(string $type, string $condition, array|NULL $variables = NULL, string|NULL $prefix = NULL)
  {
    $propertyName = $type === 'WHERE' ? 'where' : 'having';
    if (!isset($variables) || count($variables) < 1) {
      $this->{$propertyName} .= " " . ($prefix ?? $type) . " " . $condition;
      return $this;
    }
    $this->{$propertyName} .= " " . ($prefix ?? $type) . " " . static::sanitizeExcerpts($condition, $variables);
    return $this;
  }

  public function addOptionalCondition(string $type, string $condition, array $variables, string|NULL $prefix = NULL)
  {
    $propertyName = $type === 'WHERE' ? 'where' : 'having';
  ...
  public function where(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('WHERE', $condition, $variables);
  }

  public function orWhere(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('WHERE', $condition, $variables, 'OR');
  }

  public function andWhere(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('WHERE', $condition, $variables, 'AND');
  }

  public function optionalWhere(string $condition, array $variables)
  {
    return $this->addOptionalCondition('WHERE', $condition, $variables);
  }

  public function orOptionalWhere(string $condition, array $variables)
  {
    return $this->addOptionalCondition('WHERE', $condition, $variables, 'OR');
  }

  public function andOptionalWhere(string $condition, array $variables)
  {
    return $this->addOptionalCondition('WHERE', $condition, $variables, 'AND');
  }

  public function having(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('HAVING', $condition, $variables);
  }

  public function orHaving(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('HAVING', $condition, $variables, 'OR');
  }

  public function andHaving(string $condition, array|NULL $variables = NULL)
  {
    return $this->addCondition('HAVING', $condition, $variables, 'AND');
  }

  public function optionalHaving(string $condition, array $variables)
  {
    return $this->addOptionalCondition('HAVING', $condition, $variables);
  }

  public function orOptionalHaving(string $condition, array $variables)
  {
    return $this->addOptionalCondition('HAVING', $condition, $variables, 'OR');
  }

  public function andOptionalHaving(string $condition, array $variables)
  {
    return $this->addOptionalCondition('HAVING', $condition, $variables, 'AND');
  }
  ...
  {
    $query = $this->wrapper_start . "SELECT " . $this->columns . $this->table . $this->joins . $this->where . $this->group_by . $this->having . $this->order_by . $this->limit . $this->wrapper_end;
    return $query;
  }
